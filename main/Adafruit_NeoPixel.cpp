...
int8_t IRAM_ATTR digitalWrite(uint8_t pin, bool val) {
...
Adafruit_NeoPixel::~Adafruit_NeoPixel() {
  ...
  if(pin >= 0) gpioDefine(pin, GPIOMode::INPUT);
}
...
void Adafruit_NeoPixel::setPin(uint8_t p) {
  if(begun && (pin >= 0)) gpioDefine(pin, GPIOMode::INPUT);
    pin = p;
    if(begun) {
      gpioDefine(p, GPIOMode::OUTPUT);
      digitalWrite(p, 0);
    }
}
